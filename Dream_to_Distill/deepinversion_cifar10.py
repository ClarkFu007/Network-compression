...
def get_images(net, bs=256, epochs=1000, idx=-1, var_scale=0.00005,
               ...
               use_amp=False, optimizer=None, inputs=None, bn_reg_scale=0.0, random_labels=False, l2_coeff=0.0):
    """
    ...
    args in:
        ...
        use_amp: boolean to indicate usage of APEX AMP for FP16 calculations - twice faster and less memory
                 on TensorCores.
    ...
    kl_loss = nn.KLDivLoss(reduction='batchmean').cuda()  # Kullback-Leibler divergence
    ...
    # initialize gaussian inputs: bs images (32*32*3)
    ...
    for module in net.modules():  # The network to be inverted.
    ...
    for epoch in range(epochs):
        ...
        inputs_jit = torch.roll(inputs, shifts=(off1, off2), dims=(2, 3))  # shape: (bs, 3, 32, 32)
        ...
        if competitive_scale != 0.0:
            ...
            if 1:
                ...
                eps = 0.0
                # loss_verifier_cig = 0.5 * kl_loss(F.log_softmax(outputs_verifier / T, dim=1), M) +
                # 0.5 * kl_loss(F.log_softmax(outputs/T, dim=1), M)
                loss_verifier_cig = 0.5 * kl_loss(torch.log(P + eps), M) + 0.5 * kl_loss(torch.log(Q + eps), M)
                # JS criteria: 0 means full correlation, 1 means completely different
                loss_verifier_cig = 1.0 - torch.clamp(loss_verifier_cig, 0.0, 1.0)  # R_complete

                loss = loss + competitive_scale * loss_verifier_cig

        # Apply total variance regularization
        # :-1 - except the last one, 1: - except the first one
        diff1 = inputs_jit[:, :, :, :-1] - inputs_jit[:, :, :, 1:]
        diff2 = inputs_jit[:, :, :-1, :] - inputs_jit[:, :, 1:, :]
        diff3 = inputs_jit[:, :, 1:, :-1] - inputs_jit[:, :, :-1, 1:]
        diff4 = inputs_jit[:, :, :-1, :-1] - inputs_jit[:, :, 1:, 1:]
        loss_var = torch.norm(diff1) + torch.norm(diff2) + torch.norm(diff3) + torch.norm(diff4)
        loss = loss + var_scale * loss_var  # The total vaiance regularization coefficient

        # R_feature loss: bn_reg_scale - weight for BN regularization statistic
        loss_distr = sum([mod.r_feature for mod in loss_r_feature_layers])
        loss = loss + bn_reg_scale * loss_distr  # best for noise before BN

        # l2 loss
        if 1:  # L2 regularization coefficient
            loss = loss + l2_coeff * torch.norm(inputs_jit, 2)

        import torchvision.utils as vutils
        if debug_output and epoch % 200 == 0:
            print(f"It {epoch}\t Losses: total: {loss.item():3.3f},\ttarget: {loss_target:3.3f} \tR_feature_loss "
                  f"unscaled:\t {loss_distr.item():3.3f}")
            """
            ...
            """
            vutils.save_image(inputs.data.clone(),
                              '/home/yaof/apex/deep_dream/{}/output_{}.png'.format(prefix, epoch//200),
                              normalize=True, scale_each=True, nrow=10)
    ...
    if idx == 0:
        print('Teacher correct out of {}: {}, loss at {}'
              .format(bs, predicted_teach.eq(targets).sum().item(), criterion(outputs, targets).item()))
        print('Student correct out of {}: {}, loss at {}'
              .format(bs, predicted_std.eq(targets).sum().item(), criterion(outputs_student, targets).item()))
    ...
    vutils.save_image(best_inputs[:20].clone(),
                      '/home/yaof/apex/deep_dream/{}/output_{}.png'.format(name_use, next_batch),
                      normalize=True, scale_each=True, nrow=10)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cig_scale', default=0.0, type=float, help='competition scale')
    parser.add_argument('--di_lr', default=0.1, type=float, help='learning rate for deep inversion')
    parser.add_argument('--di_var_scale', default=2.5e-5, type=float, help='total vaiance regularization coefficient')
    ...
    parser.add_argument('--teacher_weights', default="/home/yaof/apex/deep_dream/cache/models/teacher",
                        type=str, help='path to load weights of the teacher model')
    ...
    """
        ...
        Additionally install APEX library for FP16 support (2x less memory and 2x faster).
    half: 16-bit floating point; float: 32-bit floating point
    """
    data_type = torch.half if args.amp else torch.float
    """
        The "args.bs" is the "batch size".
    ...
    optimizer_di = optim.Adam([inputs], lr=args.di_lr)  # The learning rate for deep inversion
    ...
    if args.amp:
        ...
        [net_student, net_teacher], optimizer_di = \
            amp.initialize([net_student, net_teacher], optimizer_di,
                           opt_level=opt_level, loss_scale=loss_scale)

    """
        My guess is, you used torch.save(model) (saves the entire model with architecture and 
    weights and probably other parameters) instead of torch.save(model.state_dict()) (saves only 
    the state dict, i.e. the weights).If you need to stick to the first approach, then you must 
    load it as follows: model = torch.load(PATH).
    """
    checkpoint = torch.load(args.teacher_weights)
    net_teacher.load_state_dict(checkpoint)
    # net_teacher = torch.load(args.teacher_weights)
    net_teacher.eval()  # important, otherwise generated images will be nonnatural
    if args.amp:
    ...
    batch_idx = 0
    # "paper_parameters" or"paper_parameters_better"
